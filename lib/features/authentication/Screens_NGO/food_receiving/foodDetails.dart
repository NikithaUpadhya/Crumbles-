                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          if (postData['status'] == 'Pending') {
                            ...
                            // Update the status to "Pending" and add current user ID to the NGOs subcollection
                            await FirebaseFirestore.instance.runTransaction((transaction) async {
                              DocumentReference postRef = FirebaseFirestore.instance.collection('posts').doc(postId);
                              transaction.update(postRef, {'status': 'Pending'});

                              DocumentReference ngoRef = postRef.collection('NGOs').doc(currentUserId);
                              transaction.set(ngoRef, {'userId': currentUserId});
                            });
